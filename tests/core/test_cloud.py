...
def test_is_cloud_enabled_false_when_in_emulator(clean_env):
    ...
    with patch.dict(
        os.environ, {"FUNCTIONS_EMULATOR": "true", PDD_JWT_TOKEN_ENV: "ey.test.token"}
    ):
        assert CloudConfig.is_cloud_enabled() is False
...
def test_is_cloud_enabled_false_when_force_local_with_jwt(clean_env):
    ...
    with patch.dict(
        os.environ, {"PDD_FORCE_LOCAL": "1", PDD_JWT_TOKEN_ENV: "ey.test.token"}
    ):
        assert CloudConfig.is_cloud_enabled() is False
...
def test_is_cloud_enabled_true(clean_env):
    ...
    with patch.dict(
        os.environ, {FIREBASE_API_KEY_ENV: "key", GITHUB_CLIENT_ID_ENV: "id"}
    ):
...
def test_noninteractive_auth_never_starts_device_flow(
    ...
    with patch.dict(
        ...
        with patch(
            "pdd.core.cloud.FirebaseAuthenticator._get_stored_refresh_token",
            return_value=None,
        ):
...
@patch("pdd.core.cloud._get_cached_jwt", return_value=None)
@patch("pdd.core.cloud.device_flow_get_token", new_callable=AsyncMock)
def test_get_jwt_token_unexpected_error(
...
@patch("pdd.core.cloud._get_cached_jwt")
def test_get_jwt_token_in_async_context_with_cached_token(
    mock_get_cached_jwt, clean_env
):
...
@patch("pdd.core.cloud._get_cached_jwt")
def test_get_jwt_token_in_async_context_without_cached_token(
    mock_get_cached_jwt, clean_env
):
...
def test_async_context_auth_error_references_correct_command(clean_env):
    ...
    with patch.dict(
        ...
        with (
            patch("pdd.core.cloud.asyncio.get_running_loop", return_value=MagicMock()),
            patch("pdd.core.cloud._get_cached_jwt", return_value=None),
